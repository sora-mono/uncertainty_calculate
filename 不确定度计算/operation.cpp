...
operation::operation(long double real, long effective_digits, long uncertain_fin, long uncertain_digits) :realpart(effective_digits, real), uncertainpart(-1, uncertain_fin, uncertain_digits, -1, -1)
{
...
operation operation::operator^(const long double k)const
{
	reality temp_reality = reality(realpart.get_effective_digits_inside(), pow(realpart.get_real(), k));
...
operation sin(const operation& node)
{
	...
	long double uncertain_fin = node.uncertainpart.get_uncertain_fin() * pow(long double(10), node.uncertainpart.get_uncertain_fin_digits());
